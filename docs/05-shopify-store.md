...
## 5. 테마 — 히어로 섹션 반영 (2026-08-25)

작업 대상 테마는 **`Marn5 JP — dev` (`156495577242`, UNPUBLISHED)** 다.
라이브 테마(`Horizon`, `156492169370`)는 건드리지 않았다.
MCP 는 라이브 테마 쓰기를 차단하므로 구조적으로도 안전하다.

| 확인 | 링크 |
|---|---|
| 미리보기 | `https://cwx1ii-ku.myshopify.com/?preview_theme_id=156495577242` |
| 테마 에디터 | `https://admin.shopify.com/store/cwx1ii-ku/themes/156495577242/editor` |

비밀번호 보호는 꺼져 있어 링크만으로 열린다.

### 넣은 파일

| 파일 | 내용 |
|---|---|
| `sections/marn5-hero.liquid` | 히어로 섹션. `design/jp-header-hero.html` 이식 |
| `templates/index.json` | 홈 = 히어로 + 상품 그리드. Horizon 기본 hero 를 대체 |
| `assets/hero-a.mp4` · `hero-b.mp4` | 브랜드스토리 영상 2안 |
| `assets/hero-a-poster.jpg` · `hero-b-poster.jpg` | 포스터 |

### 테마 에디터에서 바꿀 수 있는 값

숫자를 코드에서 고치지 않고 에디터에서 돌려 볼 수 있게 뺐다.

- 배경 영상 A안/B안 · 포스터
- 높이 60~100vh (기본 88)
- **각 반경 0~60px (기본 36)** — 레퍼런스 준수 지점
- 백색 오버레이 농도 0~100% (기본 78)
- 헤드라인 2행 · 서브카피
- 관세 고지 카드 문구 (기본 ON)
- 상품 카드 (상품 선택 + 대체 표시값)

**팔레트는 에디터에 노출하지 않았다.** §11 에서 액센트 컬러 없음이 확정이라
에디터에 색을 열어 두면 그 결정이 조용히 깨진다.

### 알려진 제약

- 41개 상품이 전부 **DRAFT** 라 스토어프론트에서 `section.settings.product` 가
  비어 온다. 카드가 통째로 사라지지 않도록 **대체 표시값**을 넣어 뒀다.
  상품이 ACTIVE 가 되면 자동으로 실제 데이터로 바뀐다.
- **헤더는 아직 Horizon 기본**이다. `sections/header.liquid` 이 54KB 라
  통째로 바꾸는 대신 히어로를 먼저 올려 검토받는다.
- **폰트가 아직 기본값**이다. Schibsted Grotesk + Zen Kaku Gothic New 미적용.
- 모바일은 PC 가로 영상을 4:5 로 자른다. 세로 크롭본 미제작.
